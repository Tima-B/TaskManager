...
def get_perms(request: HttpRequest | Request, obj: Model = None) -> dict:
    ...
    # ищем данные о модели, id объекта и заносим в словарь
    kw = {}
    ...
    # если объект - инстанс модели, то попытка найти объект по данным из url
...
def gen_data(cnt=0, close=0, clear=False, parent=False, clear_user=False) -> None:
    ...
    user_start = getattr(settings, "MY_TEST_USER", "test_user")
    user_pass = getattr(settings, "MY_TEST_PASS", "password")
    ...
    for i in range(full + 1, int(cou) + full + 1):
        author = choice(users)
        obj = Proj()
        obj.name = f"Тестовый проект №{i}"
        obj.desc = f"Описание тестового проекта №{i}"
        obj.author = author
        obj.uweb = author
    ...
    for i in range(full + 1, int(cou) + full + 1):
        author = choice(users)
        obj = Sprint()
        obj.name = f"Тестовый спринт №{i}"
        obj.desc = f"Описание тестового спринта №{i}"
        obj.author = author
        obj.uweb = author
    ...
    for i in range(full + 1, int(cou) + full + 1):
        ...
        user = choice(users)
        obj = Task()
        obj.name = f"Тестовая задача №{i}"
        obj.desc = f"Описание тестовой задачи №{i}"
        obj.author = author
        obj.uweb = author
        obj.user = user
        obj.date_beg = beg + timedelta(randint(0, step))
