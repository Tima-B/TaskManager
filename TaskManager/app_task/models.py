...
class Proj(models.Model):
    ...
    def save(self, **kwargs) -> None:
        ...
        # проверка и корректировка плановой даты окончания
        if self.date_max and self.date_max < self.date_beg:
            self.date_max = self.date_beg

        # при наличии не закрытых задач/спритов обнуляем дату завершения
        if (
        ...
        # корректировка даты завершения проекта
        ...
        # при наличии изменений сохраняем и создаём запись в истории
        if desc:
            super().save(**kwargs)
...
class Sprint(models.Model):
    ...
    def save(self, **kwargs) -> None:
        ...
        # если проект закрыт, то не сохраняем
        ...
        # при наличии не закрытых задач обнуляем дату завершения
        if Task.objects.filter(sprint_id=self.id).filter(date_end=None).exists():
            self.date_end = None

        # корректировка даты завершения спринта, если меньше даты закрытия проекта
        ...
        if desc:
            ...
            # Корректировка id проекта, при на несовпадение проекта с проектом спринта
...
class Task(models.Model):
    ...
    def save(self, **kwargs) -> None:
        ...
        if desc:
            ...
            # Правка спринта и проекта у зависимых задачи с записью в историю
